...
async function findUnusedFiles() {
    ...
    const sourceContent = await Promise.all(
        srcFiles.map(file => 
            fs.readFile(path.join(srcDir, file), 'utf8')
                .catch(() => '')
        )
    );
    ...
    for (const file of allFiles) {
        ...
        if (allContent.includes(basename) || 
            allContent.includes(relativePath)) {
...
function isEssentialFile(file) {
    ...
    return essentialFiles.includes(path.basename(file)) ||
           essentialPaths.some(p => file.includes(p));
}
